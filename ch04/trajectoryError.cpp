...
string openTrajectoryFile(const string& filename) {
    ifstream test(filename);
    if (test) return filename;
    const vector<string> candidates = {"../ch04/" + filename, "ch04/" + filename};
...
int main(int argc, char** argv) {
    TrajectoryType groundtruth = ReadTrajectory(openTrajectoryFile("groundtruth.txt"));
    TrajectoryType estimated = ReadTrajectory(openTrajectoryFile("estimated.txt"));
...
TrajectoryType ReadTrajectory(const string &path) {
    ...
    if(!fin) {
        cerr << "Trajectory file not found!" << endl;
        return trajectory;
    }
