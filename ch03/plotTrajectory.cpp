...
int main(int argc, char** argv) {
    string trajectory_file = argc > 1 ? argv[1] : "trajectory.txt";
    ifstream fin(trajectory_file);

    if (!fin) {
        const vector<string> candidates = {"../ch03/trajectory.txt", "ch03/trajectory.txt"};
    ...
    if (!fin) {
        cout << "trajectory file not found. Tried: trajectory.txt";
        for (const auto& path : {"../ch03/trajectory.txt", "ch03/trajectory.txt"}) {
            cout << ", " << path;
        }
        cout << endl;
        return 1;
